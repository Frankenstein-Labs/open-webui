...
def choose_engine(form_data: dict[str, Any]) -> EngineDecision:
    ...
    explicit = (
        metadata.get("engine")
        or form_data.get("engine")
        or os.getenv("OPEN_WEBUI_ENGINE")
        or "auto"
    ).lower()
    ...
    text = " ".join(
        ...
        for item in form_data.get("messages", [])
...
async def _run_ai_manus(form_data: dict[str, Any]) -> str:
    ...
    async with websockets.connect(ws_url, additional_headers=headers or None) as socket:
        await socket.send(json.dumps({"id": "open-webui-join", "version": 2, "type": "join_session", "session_id": session_id}))
        await socket.recv()
